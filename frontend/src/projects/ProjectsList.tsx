...
import { getClientDisplayName } from '../clients/utils';
...
const ProjectsList: React.FC<ProjectsListProps> = ({
  ...
  useEffect(() => {
    loadProjects();
  }, [sortModel, paginationModel, selectedStatuses, clientId]);
      ...
      ? [
          {
            ...
            valueGetter: (params: any) =>
              params.row.client ? getClientDisplayName(params.row.client) : '',
          },
        ]
      : []),
    {
        ...
        >
          ...
          {showAddButton && (
            <PrimaryButton
              startIcon={<AddIcon />}
              onClick={() =>
                navigate(clientId ? `/projects/add?client_id=${clientId}` : '/projects/add')
              }
            >
              {clientId
                ? intl.formatMessage({ id: 'projects.client.add' })
      ...
      <Paper sx={{ p: 2, mb: 3, borderRadius: '10px' }}>
        <Box
          sx={{
            ...
            gridTemplateColumns: {
              xs: '1fr',
              md: clientId ? '2fr 1fr auto' : '2fr 1fr 1fr auto',
            },
            gap: 2,
            alignItems: 'flex-end',
          }}
        >
          ...
          <TextField
            ...
            placeholder={
              clientId
                ? intl.formatMessage({ id: 'projects.search.placeholderClient' }) ||
                  'Rechercher par nom de projet...'
                : intl.formatMessage({ id: 'projects.search.placeholder' })
            }
            value={searchTerm}
